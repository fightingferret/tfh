...
void do_tell( char_data *ch, char *argument )
{
  ...
  if( ch->pcdata != NULL && victim->pcdata != NULL ) {
    ...
    // Check to see if victim is invis imm.  Simulate err msg from not_found.
    wizard_data* imm = wizard(victim);
    if ( imm != NULL && is_set( victim->pcdata->pfile->flags, PLR_WIZINVIS )
                     && get_trust( ch ) < imm->wizinvis ) {
       fsend( ch, "Nothing found matching '%s'.", arg );
       return;
    }
  }

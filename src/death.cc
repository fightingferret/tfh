...
void 
loot_corpse( obj_data* corpse, char_data* ch, char_data* victim )
{
  if( ch == NULL || ch->pcdata == NULL || victim->species == NULL || 
      corpse == NULL ) 
    return;
  ...
  case 2: // loot coins
    ...
    for ( int jj = 0 ; jj < corpse->contents.size ; jj++ ) {
       if ( (loot_obj = object( corpse->contents[jj] )) != NULL &&
            loot_obj->pIndexData->item_type == ITEM_MONEY ) { 
	  *loot_array += (thing_data*)loot_obj ;
          for( int ii = 0; ii < MAX_COIN; ii++ ) {
             if( loot_obj->pIndexData->vnum == coin_vnum[ii] ) {
                amount += loot_obj->number * coin_value[ii];
             }
          }
       }
    }
    get_obj( ch, loot_array, corpse ); // should delete loot_array
    break;
          ...
          *loot_array += (thing_data*)loot_obj ;
          ...
          if( is_set( ch->pcdata->pfile->flags, PLR_AUTO_SPLIT ) &&
             ...
             for( int i = 0; i < MAX_COIN; i++ ) {
                if( loot_obj->pIndexData->vnum == coin_vnum[i] ) {
                   amount += loot_obj->number * coin_value[i];
                }
             }
          }
       }
...
void do_slay( char_data* ch, char* argument )
{
 ...
 mortal form.\n\r" );
  ...
  if( ( victim = one_character( ch, argument, "slay", ch->array,  
  ...
  make_corpse( victim, ch->array ); // corpse goes to slayer's room
  //make_corpse( victim, victim->array ); // corpse goes to victim's room

...
void open_object( char_data* ch, obj_data* obj )
{
  ...
  int i;
  obj_data* trap = NULL;
  oprog_data* oprog = NULL;
  for ( i = obj->contents.size - 1; i >= 0; i-- ) {
     trap = object( obj->contents[i] );
     if ( trap == NULL )
        continue;
     if ( trap->pIndexData->item_type == ITEM_TRAP && trap->Seen( ch ) ) {
        // Execute the trap's USE trigger.  This will probably detonate the
        // trap, and may well extract it (thus affecting obj->contents array).
...
void do_pick( char_data* ch, char* argument )
{
  ...
  obj_data* lockpick = one_object( ch, "lock_pick", empty_string,

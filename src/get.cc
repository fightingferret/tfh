...
void do_get( char_data* ch, char* argument )
{
  ...
  if( !two_argument( argument, "from", arg ) ) {
    ...
    if( (array = several_things( ch, argument, "get", ch->array )) == NULL ) 
       return;
  ...
  if( ( thing = one_object( ch, argument, "get from", &ch->wearing, 
  ...
  if( ( array = several_things( ch, arg, "get", &thing->contents ) ) == NULL )
    return;

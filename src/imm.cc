...
void do_echo( char_data *ch, char *argument )
{
  ...
  for( link = link_list; link != NULL; link = link->next )
    if( link->connected == CON_PLAYING ) {
      //fsend( link->character, argument ); // old code
...
void do_where( char_data* ch, char* )
{
  ...
  // List all players in the same area.  Assume that we can see all players.
  // (i.e., don't test victim->Seen(ch) ).
  for ( link = link_list; link != NULL; link = link->next ) 
  {
      if ( link->connected == CON_PLAYING ) 
      {
          victim = link->character;
          if ( victim != NULL && !IS_NPC( victim ) && victim->in_room != NULL
...
void do_snoop( char_data* ch, char* argument)
{
  ...
  if( ( victim = one_character( ch, argument, "snoop", ch->array,
			       (thing_array*)&player_list,
			       (thing_array*)&mob_list ) ) == NULL ) {

...
bool Char_Data :: Seen( char_data* ch )
{
  ...
  if ( !tfh_assume_lit ) {
     if( in_room != NULL && in_room->is_dark( )
         && !is_set( affected_by, AFF_SANCTUARY )
         && ( !is_set( ch->affected_by, AFF_INFRARED ) ||
              ( species != NULL && !is_set( &species->act_flags, ACT_WARM_BLOODED ) ) ) )
        return FALSE;
  }
...
void do_peek( char_data* ch, char* argument )
{
  ...
  if( ch->shdata != NULL && ch->shdata->skill[ SKILL_PEEK ] == UNLEARNT ) {
...
void show_room( char_data* ch, room_data* room, bool brief, bool scan )
{
  ...
  ** or room contents! */
  if ( room->Seen( ch ) ) {
	...
	// 5/11: If thing is a character, call Char_Data::Seen(ch) to see if 
        // the player can see it.  If not, go to next thing in room.
	rch = character(thing);
	if ( rch != NULL && !rch->Seen(ch) ) {
	   continue;
	}
  ...
  } // end if character can see the room

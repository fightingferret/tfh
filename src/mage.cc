                           ...
                           int level, int duration )
{
  ...
  for( int i = ch->array->size - 1; i >= 0; i-- ) {
    ...
    if( victim == ch || 
	!victim->Can_See( ) || 
        ( victim->species != NULL && !is_set( &victim->species->act_flags, 
					     ACT_HAS_EYES ) ) )
      continue;
    ...
    else if( !can_kill( ch, victim ) ) {
      send( victim, "Oddly the flash has no affect on you.\n\r" );
    }
    else {
      spell_affect( ch, victim, level, duration, SPELL_BLINDING_LIGHT, 
                    AFF_BLIND );
    }
  }

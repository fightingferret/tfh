...
void remove_reagent( obj_data* reagent, char_data* ch )
{
  ...
  if( reagent->number > 1 ) {
    reagent->number--; 
    if ( reagent->array != NULL ) {  // fix char's total # objects in inventory
       reagent->array->number--;
    }
  ...
  obj_data* reagent )
{
  ...
  if( target != NULL && victim != NULL 
     ...
     if( victim->position > POS_SLEEPING )
         act( victim, spell_act_table[ action ].victim_other, ch, victim, 
	      reagent );
     act_neither( spell_act_table[ action ].others_other, ch, victim, 
		  reagent );
  }
  else { // target isn't a character
     act_notchar( spell_act_table[action].others_other, ch, NULL, reagent,
                  target );
  }
}

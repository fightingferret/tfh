  ...
  if ( !can_sneak(ch) ) {
    for( int i = 0; i < room->contents; i++ ) {
      if( ( rch = character( room->contents[i] ) ) != NULL &&
          rch != ch && rch->link != NULL && rch->position > POS_SLEEPING && 
          !group_message( rch, ch ) ) {
        if( ch->Seen( rch ) ) {
          if( ch->mount != NULL ) 
            send( rch, "%s riding %s has arrived.\n\r", ch, ch->mount );
          else 
            send( rch, "%s %s from %s.\n\r", ch, arriving_verb[type],
                      ...
                      int type,  action_data* action )
{
  ...
  if ( !can_sneak(ch) ) {
     for( int i = 0; i < room->contents; i++ ) {
        if( ( rch = character( room->contents[i] ) ) != NULL && 
            ch != rch && rch->link != NULL && rch->position > POS_SLEEPING && 
            !group_message( rch, ch ) ) {
...
void leaving_self( char_data* ch, exit_data* exit, int type, 
                   action_data* action )
{
  if( ch->mount != NULL ) {
    ...
    send( ch, "You %s through %s!\n\r",
          exit->direction == DIR_UP || exit->direction == DIR_DOWN ? "climb" :
                                                                     "step",
          exit->name );      
  }
  else {
    act( ch, prog_msg( action, leaving_msg[0], leaving_msg[1] ),
         ch, NULL, leaving_action[type], dir_table[ exit->direction ].name );
  }
                    ...
                    exit_data* exit, int type, action_data* action )
{
  ...
  if( is_set( &exit->exit_info, EX_CLOSED ) ) {
    send( rch, "%s %s %s passing through %s!\n\r", ch, leaving_verb[type], 
          dir_table[ exit->direction ].name, exit->name );
  }
  else if( ch->mount == NULL ) {
     act( rch, prog_msg( action, leaving_msg[2], leaving_msg[3] ), ch, NULL,
          leaving_verb[type], dir_table[ exit->direction ].name );
  }
  else {
    ...
    if( ( rch = character( ch->array->list[i] ) ) != NULL ) {
      ...
      if( includes( rch->aggressive, ch )
        ...
        send( ch, "%s %s blocking your exit %s.\n\r", rch,
	      (rch->shown == 1 ? "is" : "are"), dir_table[door].name );
        return FALSE;
        }
      }    
    }
                      ...
                      action_data*& action )
{
  ...
  for( int i = 0; i < *ch->array; i++ ) { 
    if( ( npc = mob( ch->array->list[i] ) ) == NULL || npc->pcdata != NULL ||
        npc->position < POS_RESTING || npc == ch || !npc->Can_See() )
      continue; 
...
bool trigger_entering( char_data *ch, room_data *room, int door )
{
  ...
  for( int i = 0; i < room->contents; i++ ) {
    if( ( npc = mob( room->contents[i] ) ) == NULL || npc == ch || 
        npc->position < POS_RESTING || !npc->Can_See() )
      continue;
    for( mprog = npc->species->mprog; mprog != NULL; mprog = mprog->next ) 
...
void do_search( char_data* ch, char* argument )
{
  ...
  if ( ch->shdata->skill[SKILL_SEARCHING] != UNLEARNT ) {
     if( toggle( ch, argument, "Searching", ch->pcdata->pfile->flags, 
                 PLR_SEARCHING ) )
        return;
  }

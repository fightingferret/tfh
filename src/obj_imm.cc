...
const char* material_name [] = { "paper", "wood",
  ...
  "unused", "bronze", "iron", "steel", "mithril", "liridium",
...
void do_oedit( char_data* ch, char* argument )
{
  ...
  if( matches( argument, "new" ) ) {
    ...
    // SHOULD have two possible obj_clss_data constructors in object.cc:
    // obj_clss_data::obj_clss_data( const char* name, int vnum, 
    //                               const char* creator );
    // obj_clss_data::obj_clss_data( obj_clss_data* obj_copy, const char* name,
    //                               int vnum, const char* creator );
    // Then this code calls one of the two constructors.
...
void do_ostat( char_data* ch, char* argument )
{
  ...
  if( obj != NULL ) {
    ...
    case ITEM_ARMOR :
        page( ch, "   Enchantment: %-9d Protection Aura: %d\n\r\n\r",
    ...
    case ITEM_DRINK_CON:
        page( ch, "      Fullness: %-10d  Liquid: %d (%s)\n\r", 
	      obj->value[1], obj->value[2], 
             (obj->value[2] < 0 || obj->value[2] >= MAX_ENTRY_LIQUID) ? "none" :

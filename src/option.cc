...
const char *iflag_name [] = { "auction", "info.clans", "deaths", "levels",
  "logins", "info.bugs", "requests", "writes", "notes" };
...
void do_options( char_data* ch, char* argument )
{
  ...
  for( i = 0; i < MAX_PLR_OPTION; i++ ) {
    if( matches( argument, plr_name[i] ) ) {
      ...
      if ( (i == PLR_SEARCHING && 
                            ch->shdata->skill[SKILL_SEARCHING] == UNLEARNT ) ||
           (i == PLR_SNEAK && ch->shdata->skill[SKILL_SNEAK] == UNLEARNT ) ||
           (i == PLR_TRACK && ch->shdata->skill[SKILL_TRACK] == UNLEARNT ) )
      {

...
void do_inspect( char_data* ch, char* argument )
{
  ...
  if ( IS_NPC( ch ) || ch->shdata->skill[ SKILL_INSPECT ] == UNLEARNT ) {
  ...
  for ( int i = 0; i < obj->contents.size; i++ ) {
      ...
      if ( item == NULL ) 
	 continue;
      if ( item->pIndexData->item_type == ITEM_TRAP && item->Seen( ch ) && 
           item->pIndexData->level <= ch->shdata->level && 
...
void do_untrap( char_data *ch, char *argument )
{
  ...
  if ( IS_NPC( ch ) || ch->shdata->skill[ SKILL_UNTRAP ] == UNLEARNT ) {
    send( ch, "You really don't know how to remove traps.\n\r" );
    return;
  }

  if ( ch->in_room == NULL ) {
     //roach("Character trying to disarm while not in a room.", ch);
     return;
  }

  obj_data* obj;
  if( (obj = one_object( ch, arg, "remove a trap", ch->array )) == NULL ) {
...
void do_appraise( char_data *ch, char *argument )
{
  ...
  if( a_player == NULL || ch->shdata->skill[ SKILL_APPRAISE ] == UNLEARNT ) {
...
void do_bandage( char_data *ch, char *argument )
{
  ...
  if( ch->check_skill( SKILL_BANDAGE ) ) {
    ...
    send( ch,
      ...
      victim );
    act_neither( "$n tries to bandage $N's wounds but does more harm than good.",
         ch, victim, obj );
    }

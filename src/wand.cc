...
void do_zap( char_data* ch, char* argument )
{
  ...
  if ( wand->position != WEAR_HELD_L &&
       wand->position != WEAR_HELD_R ) {
  ...
  if( victim == NULL ) {
    ...
    send_seen( ch, "%s zaps %s at %sself.", ch, wand, ch->Him_Her( ) );
  }
  ...
  ( *spell_table[ spell ].function )( ch, victim, (room == NULL ? obj : room), 
				      level, duration ); 
...
void do_quaff( char_data* ch, char* argument )
{
  ...
  ( *spell_table[ spell ].function )( NULL, ch, NULL, level, duration );

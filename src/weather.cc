...
const int days_in_month [ 12 ] = { 31, 28, 31, 30, 31, 30, 
			           31, 31, 30, 31, 30, 31 }; // no leap year :)
...
void time_update( void )
{
  ...
  weather.minute += 6; // accelerate time by more (1 min r.t. = 1 hour on mud)
  //weather.minute += 1; // old code did this - too slow!
  ...
  if( *tmp != '\0' ) {
    for( link = link_list; link != NULL; link = link->next ) {
	  ...
	  IS_OUTSIDE( link->character ) && 
          IS_AWAKE( link->character ) && 
	  link->character->Can_See() && 
